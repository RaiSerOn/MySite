<?php
defined('lock_key') or die('Доступ запрещён!');  
$error_img = array();
 
if($_FILES['upload_avatar']['error'] > 0)
{
...
$newfilename = $_POST["form_brand"].'-'.$id.rand(10,100).'.'.$imgext;
...
if (move_uploaded_file($_FILES['upload_image']['tmp_name'], $uploadfile))
{
  ...
  $update = mysql_query("UPDATE cars SET Image='$newfilename' WHERE ID = '$id'",$link);   
