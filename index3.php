...
<html lang="ru">
 ...
 <div class="container searcher">
    ...
    <div class="modal fade" id="myModal" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
      <div class="modal-dialog">
        <div class="modal-content">
          ...
          <div class="modal-body">
            <div id="search">
            <form method="GET" action="search_filter.php" class = "group-search" id="group-search-modal">
              <table>
             <?php
                ...
                if(mysql_num_rows($result) > 0){
                  ...
                  do{
                      echo' <a href="view_cat.php?maincat='.$row["main_category"].'">'.$row[main_category].'</a> ';
                ...
                <tr><td><hr style="border-width: 5px;">
                        <p><strong>Групповой поиск по каталогу</strong></p>
                          <?php
                            ...
                            if(mysql_num_rows($result) > 0){
                              $row = mysql_fetch_array($result);
                              echo'<p><select class="btn btn-default" data-style="btn-primary" name="main_category">';
                              echo'<option value="default">Категория</option>';
                              do{
                                if($row[main_category] != "")
                                  echo'<option value="'.$row[main_category].'">'.$row[main_category].'</option>';
                              }while($row = mysql_fetch_array($result));
                            }
                            echo'</select></p>';
                            ...
                            if(mysql_num_rows($result) > 0){
                              ...
                              echo'<option value="default">Подкатегория</option>';
                              do{
                                if($row[category] != "")
                            ...
                            if(mysql_num_rows($result) > 0){
                              ...
                              echo'<option value="default">Масштаб</option>';
                              do{
                            ...
                            if(mysql_num_rows($result) > 0){
                              ...
                              echo'<option value="default">Марка</option>';
                              do{
                                if($row[mark] != "")
                            ...
                            if(mysql_num_rows($result) > 0){
                              ...
                              echo'<option value="default">Производитель</option>';
                              do{
                                if($row[brand] != "")
    ...
    <div class="container-fluid shop-index3">
  		<div class="NewModel block_with_cars" id="NewModel">
      <div>
      <h1 style="">Новые поступления</h1>
      <ul id="sorting-lisr">
        <a class="btn btn-primary" href="index3.php?sort=price-asc"><li>от дешёвых к дорогим</li></a>
        <a class="btn btn-primary" href="index3.php?sort=price-desc"><li>от дорогих к дешёвым</li></a>
        <a class="btn btn-primary" href="index3.php?sort=news"><li>новинки</li></a>
        <a class="btn btn-primary" href="index3.php?sort=brand"><li>от а до я</li></a>
      </ul>
        ...
        <ul>
                ...
                if($temp[0] > 0){
                  ...
                  if(empty($page) or $page < 0) $page = 1;
    				...
    				$result = mysql_query("SELECT * FROM cars WHERE visible='1' ORDER BY $sorting $qury_start_num",$link);
            ...
            if ($page != 1)  $pstr_prev = '<li><a class="btn btn-default pstr-prev" href="index3.php?sort='.$_GET["sort"].'&page='.($page - 1).'">&lt;</a></li>';
            if ($page != $total)  $pstr_next = '<li><a class="btn btn-default pstr-next" href="index3.php?sort='.$_GET["sort"].'&page='.($page + 1).'">&gt;</a></li>';
            if($page - 5 > 0) $page5left = '<li><a class="btn btn-default" href="index3.php?sort='.$_GET["sort"].'&page='.($page - 5).'">'.($page - 5).'</a></li>';
            if($page - 4 > 0) $page4left = '<li><a class="btn btn-default" href="index3.php?sort='.$_GET["sort"].'&page='.($page - 4).'">'.($page - 4).'</a></li>';
            if($page - 3 > 0) $page3left = '<li><a class="btn btn-default" href="index3.php?sort='.$_GET["sort"].'&page='.($page - 3).'">'.($page - 3).'</a></li>';
            if($page - 2 > 0) $page2left = '<li><a class="btn btn-default" href="index3.php?sort='.$_GET["sort"].'&page='.($page - 2).'">'.($page - 2).'</a></li>';
            if($page - 1 > 0) $page1left = '<li><a class="btn btn-default" href="index3.php?sort='.$_GET["sort"].'&page='.($page - 1).'">'.($page - 1).'</a></li>';

            if($page + 5 <= $total) $page5right = '<li><a class="btn btn-default" href="index3.php?sort='.$_GET["sort"].'&page='.($page + 5).'">'.($page + 5).'</a></li>';
            if($page + 4 <= $total) $page4right = '<li><a class="btn btn-default" href="index3.php?sort='.$_GET["sort"].'&page='.($page + 4).'">'.($page + 4).'</a></li>';
            if($page + 3 <= $total) $page3right = '<li><a class="btn btn-default" href="index3.php?sort='.$_GET["sort"].'&page='.($page + 3).'">'.($page + 3).'</a></li>';
            if($page + 2 <= $total) $page2right = '<li><a class="btn btn-default" href="index3.php?sort='.$_GET["sort"].'&page='.($page + 2).'">'.($page + 2).'</a></li>';
            if($page + 1 <= $total) $page1right = '<li><a class="btn btn-default" href="index3.php?sort='.$_GET["sort"].'&page='.($page + 1).'">'.($page + 1).'</a></li>';
            if($page + 5 < $total){
              $strtotal = '<li><p class="nav-point">...</p></li><li><a class="btn btn-default" href="index3.php?&sort='.$_GET["sort"].'&page='.$total.'">'.$total.'</a></li>';
            } else {
              $strtotal ="";
            }
            if($total > 1){
              echo'
                <div class="pstrnav col-lg-8 col-lg-offset-4 col-md-8 col-md-offset-4" style="height: 100px; ">
                  <ul>
              ';
              echo $pstr_prev.$page5left.$page4left.$page3left.$page2left.$page1left."<li><a class='pstr-active btn btn-default' href='index3.php?sort=".$_GET["sort"]."&page=".$page."'>".$page."</a></li>".$page1right.$page2right.$page3right.$page4right.$page5right.$strtotal.$pstr_next;
                echo'
                  </ul>
                </div>
              ';
    ...
    <script>
      $(document).ready(
          function() {
              $("html").niceScroll({cursorcolor:"#000",smoothscroll:true,mousescrollstep: 10,scrollspeed: 120});
          }
      );
    </script>
  </body>
