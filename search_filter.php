...
<div class="modal fade" id="myModal" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
  <div class="modal-dialog">
    ...
    <div class="container-fluid search-block" style="background-color: #fff; padding: 0;">
      <div class="NewModel block_with_cars" id="NewModel">
      <div>
      <h1 style="text-shadow: 1px 1px 0 rgba(0,0,0,.75); text-align: center; color: #ffffff;">Результаты поиска</h1>
      <ul id="sorting-lisr">
        <a class="btn btn-primary" href="search_filter.php?category=<?php echo $_GET["category"]?>&scale=<?php echo $_GET["scale"]?>&mark=<?php echo $_GET["mark"]?>&brand=<?php echo $_GET["brand"]?>&page=<?php echo $page?>&sort=price-asc"><li>от дешёвых к дорогим</li></a>
        <a class="btn btn-primary" href="search_filter.php?category=<?php echo $_GET["category"]?>&scale=<?php echo $_GET["scale"]?>&mark=<?php echo $_GET["mark"]?>&brand=<?php echo $_GET["brand"]?>&page=<?php echo $page?>&sort=price-desc"><li>от дорогих к дешёвым</li></a>
        <a class="btn btn-primary" href="search_filter.php?category=<?php echo $_GET["category"]?>&scale=<?php echo $_GET["scale"]?>&mark=<?php echo $_GET["mark"]?>&brand=<?php echo $_GET["brand"]?>&page=<?php echo $page?>&sort=news"><li>новинки</li></a>
        <a class="btn btn-primary" href="search_filter.php?category=<?php echo $_GET["category"]?>&scale=<?php echo $_GET["scale"]?>&mark=<?php echo $_GET["mark"]?>&brand=<?php echo $_GET["brand"]?>&page=<?php echo $page?>&sort=brand"><li>от а до я</li></a>
      </ul>
             ...
             <form method="GET" action="search_filter.php" class = "group-search">
                <p><strong>Групповой поиск по каталогу</strong></p>
                  <?php
                    ...
                    if(mysql_num_rows($result) > 0){
                      $row = mysql_fetch_array($result);
                      echo'<p><select name="category" class="btn btn-default"> ';
                      do{
                          echo'<option value="'.$row[category].'">'.$row[category].'</option>';
                      }while($row = mysql_fetch_array($result));
                    }
      ...
      <div id="start_block-list">
        <ul>
          <?php
                ...
                $count = mysql_query("SELECT COUNT(*) FROM cars WHERE visible = '1'",$link);
                ...
                $check_category = clear_string($_GET["category"]);
                $check_scale = clear_string($_GET["scale"]);
                $check_mark = clear_string($_GET["mark"]);
                $check_brand = clear_string($_GET["brand"]);
                $check_category_main = clear_string($_GET["main_category"]);
                $squery_serch = "";

                if($check_category != 'default') $squery_serch = $squery_serch . "AND Type='$check_category' "; 
                if($check_scale != 'default') $squery_serch = $squery_serch . "AND Scale='$check_scale' "; 
                if($check_mark != 'default') $squery_serch = $squery_serch . "AND Model='$check_mark' "; 
                if($check_brand != 'default') $squery_serch = $squery_serch . "AND Producer='$check_brand' "; 
                if($check_category_main != 'default') $squery_serch = $squery_serch . "AND main_TYPE='$check_category_main'"; 
                if($squery_serch == "") $squery_serch = "AND Type='default'"; 
            $result = mysql_query("SELECT * FROM cars WHERE visible='1' $squery_serch ORDER BY ID $qury_start_num",$link);
            ...
            if(mysql_num_rows($result) > 0){
              ...
              echo '<h3 style="color: #ffffff;">Поиск не дал результатов! Неверные критерии поиска!</h3>';
            }
            echo'</ul></div></div>';
            if ($page != 1)  $pstr_prev = '<li><a class="pstr-prev btn btn-default" href="search_filter.php?category='.$_GET["category"].'&scale='.$_GET["scale"].'&mark='.$_GET["mark"].'&brand='.$_GET["brand"].'&page='.($page - 1).'">&lt;</a></li>';
            if ($page != $total)  $pstr_next = '<li><a class="pstr-next btn btn-default" href="search_filter.php?category='.$_GET["category"].'&scale='.$_GET["scale"].'&mark='.$_GET["mark"].'&brand='.$_GET["brand"].'&page='.($page + 1).'">&gt;</a></li>';
            if($page - 5 > 0) $page5left = '<li><a class="btn btn-default" href="search_filter.php?category='.$_GET["category"].'&scale='.$_GET["scale"].'&mark='.$_GET["mark"].'&brand='.$_GET["brand"].'&page='.($page - 5).'">'.($page - 5).'</a></li>';
            if($page - 4 > 0) $page4left = '<li><a class="btn btn-default" href="search_filter.php?category='.$_GET["category"].'&scale='.$_GET["scale"].'&mark='.$_GET["mark"].'&brand='.$_GET["brand"].'&page='.($page - 4).'">'.($page - 4).'</a></li>';
            if($page - 3 > 0) $page3left = '<li><a class="btn btn-default" href="search_filter.php?category='.$_GET["category"].'&scale='.$_GET["scale"].'&mark='.$_GET["mark"].'&brand='.$_GET["brand"].'&page='.($page - 3).'">'.($page - 3).'</a></li>';
            if($page - 2 > 0) $page2left = '<li><a class="btn btn-default" href="search_filter.php?category='.$_GET["category"].'&scale='.$_GET["scale"].'&mark='.$_GET["mark"].'&brand='.$_GET["brand"].'&page='.($page - 2).'">'.($page - 2).'</a></li>';
            if($page - 1 > 0) $page1left = '<li><a class="btn btn-default" href="search_filter.php?category='.$_GET["category"].'&scale='.$_GET["scale"].'&mark='.$_GET["mark"].'&brand='.$_GET["brand"].'&page='.($page - 1).'">'.($page - 1).'</a></li>';

            if($page + 5 <= $total) $page5right = '<li><a class="btn btn-default" href="search_filter.php?category='.$_GET["category"].'&scale='.$_GET["scale"].'&mark='.$_GET["mark"].'&brand='.$_GET["brand"].'&page='.($page + 5).'">'.($page + 5).'</a></li>';
            if($page + 4 <= $total) $page4right = '<li><a class="btn btn-default" href="search_filter.php?category='.$_GET["category"].'&scale='.$_GET["scale"].'&mark='.$_GET["mark"].'&brand='.$_GET["brand"].'&page='.($page + 4).'">'.($page + 4).'</a></li>';
            if($page + 3 <= $total) $page3right = '<li><a class="btn btn-default" href="search_filter.php?category='.$_GET["category"].'&scale='.$_GET["scale"].'&mark='.$_GET["mark"].'&brand='.$_GET["brand"].'&page='.($page + 3).'">'.($page + 3).'</a></li>';
            if($page + 2 <= $total) $page2right = '<li><a class="btn btn-default" href="search_filter.php?category='.$_GET["category"].'&scale='.$_GET["scale"].'&mark='.$_GET["mark"].'&brand='.$_GET["brand"].'&page='.($page + 2).'">'.($page + 2).'</a></li>';
            if($page + 1 <= $total) $page1right = '<li><a class="btn btn-default" href="search_filter.php?category='.$_GET["category"].'&scale='.$_GET["scale"].'&mark='.$_GET["mark"].'&brand='.$_GET["brand"].'&page='.($page + 1).'">'.($page + 1).'</a></li>';
            if($page + 5 < $total){
              $strtotal = '<li><p class="nav-point">...</p></li><li><a class="btn btn-default" href="search_filter.php?&category='.$_GET["category"].'&scale='.$_GET["scale"].'&mark='.$_GET["mark"].'&brand='.$_GET["brand"].'&page='.$total.'">'.$total.'</a></li>';
            } else {
              $strtotal ="";
            }
            if($total > 1){
              echo'
                <div class="pstrnav" style="height: 100px;">
                  <ul>
              ';
              echo $pstr_prev.$page5left.$page4left.$page3left.$page2left.$page1left."<li><a class='pstr-active btn btn-default' href='search_filter.php?category=".$_GET["category"]."&scale=".$_GET["scale"]."&mark=".$_GET["mark"]."&brand=".$_GET["brand"]."&page=".$page."'>".$page."</a></li>".$page1right.$page2right.$page3right.$page4right.$page5right.$strtotal.$pstr_next;
                echo'
                  </ul>
                </div>
              ';
